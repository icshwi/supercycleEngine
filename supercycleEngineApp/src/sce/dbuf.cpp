...
size_t DBufPacket::size() const
{
  return dbuf.size();
}

size_t DBufPacket::size_byte() const
{
  return dbuf.size() * sizeof(dbuf.at(0));
}

std::vector<epicsUInt32> DBufPacket::getKeys() const
{
  return cmn::map1d2vec<epicsUInt32>(dbuf, 0);
}

std::vector<epicsUInt32> DBufPacket::getVals() const
{
