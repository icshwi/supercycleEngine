...
const boost::unordered_map<EVT, std::string> EVT2Str = boost::assign::map_list_of
    (C14HZ,		    "14HZ")
    (DATA,		    "DATA")
    (DATAS,		    "DATAS")
    (SEQ_END,		"SEQ_END")
...
const boost::unordered_map<DBFIDX, std::string> DBFIDX2Str = boost::assign::map_list_of
    (ProtNum,   "ProtNum")
    (ProtVer,   "ProtVer")
    (IdCycle,   "IdCycle")
    (PBState,   "PBState")
    ...
    (TgRast,    "TgRast")
    (TgSeg,     "TgSeg")
; //std::cout << " enum ProtoVer " << DBFIDX2Str.at(ProtoVer) << std::endl;
